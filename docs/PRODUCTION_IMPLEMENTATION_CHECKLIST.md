...
- [x] Keep the Website Content admin editor — it is the intended operating
  ...
  approval".)
- [ ] Do not delete already-applied migrations; first confirm which migrations
  have been applied in Supabase. (Open — requires DB access.)
...
- [ ] Apply `013_quote_request_business_details.sql` only after approval.
  (Open — migration, needs explicit confirmation.)
- [ ] Apply `014_doc_aligned_categories_and_quote_options.sql` only after
  approval. (Open — renames robotics, adds 5 quote categories, deactivates the
  legacy quote categories, rewrites `site_content` services/quote_options.)
...
- [x] Client logos (text names on the marquee now; logos pending written
  permission)
- [ ] Hallmark Cafe case study
- [ ] KEENON, ALPHA ROBOTICS, or PIMAK partnerships
...
- [ ] Request client logo files directly from ChefWare. (Open.)
- [ ] Request written permission to publish each client logo. (Open.)
- [x] Do not download or copy third-party logos from the internet. (None used.)
- [ ] Add approved logos through a controlled project/admin-assets workflow.
  (Open — pending client logos.)
...
- [ ] Apply migrations manually in Supabase, in filename order. (Open — hard
  stop without explicit in-session approval.)
- [x] Do not run migrations against production without explicit confirmation.
  (Standing rule.)
- [x] Robotics category exists. (Migration `009_add_robotics_category.sql` —
  apply pending; migration `014` renames it to "Hospitality Robotics".)
- [ ] Confirm demo products are removed and real catalogue rows exist. (`011`
  removes demo rows — apply pending; any display catalogue must be seeded by an
  admin before launch. Note: existing client-uploaded products currently sit in
  the legacy quote categories being deactivated by `014`; recategorise them to
  the new service/quote mappings.)
- [ ] Confirm the status of the site-content migrations before changing them.
  (`010`/`012` — apply pending.)
- [ ] Confirm quote business-detail columns exist. (`013` ready — apply pending.)
- [ ] Confirm `014_doc_aligned_categories_and_quote_options.sql` is applied.
  (Open — migration, needs explicit confirmation.)
- [ ] Apply `015_seed_client_catalog.sql` to populate the client catalogue.
  (Open — migration; must run after 002 and 014.)
...
- [x] Warranty & Returns copy lives in the footer. (Decision 2026-09-20:
  conservative wording pending final ChefWare approval per §6. The FAQ
  accordion was removed from the footer on 2026-09-20 at the owner's request.)
...
The remaining work is: (1) confirm with Evans which migrations 008–015 have
been applied and get approval to apply the rest; (2) apply `015_seed_client_catalog.sql` and replace the seeded placeholder prices via the Admin Inventory dashboard; (3) recategorise any client
products sitting in the legacy quote categories that `014` deactivates; (4)
seed a real display catalogue through admin; (5) promote the admin role in
Supabase; and (5) confirm the §6/§7/§10 approval items (client names, logos,
partnerships, warranty/warranty approving, delivery, payment/tax, CEO bio,
socials, legal notices) before launch.